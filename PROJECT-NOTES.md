...
| Item | Value |
|---|---|
| Deployed | **v0.5.0 DEPLOYED 2026-09-12** via release.ps1 -Force to `C:\sf2-scout\SF2 Scout.vst3` and `\\VBOXSVR\vagrant\Builds\SF2 Scout.vst3` (+ `SF2 Scout.exe`); pluginval 5 PASS. Built on branch `feat/v2` (off `feat/slot-w`; 0.2.0–0.5.0 unmerged). Cubase re-scan REQUIRED, header stamp must read 0.5.0. AWAITING user ear verdict on 0.2.0–0.5.0 (first Cubase load — `PLUGIN_CODE Sf2s` freezes on it). |
| Validator | 2026-09-12 (branch feat/v2): release.ps1 full = bundle / deploy ×2 / host pluginval 5 PASS; `dsp` = harness `test_engine` (CMake target, links `openmpt_soundlib`), **284 checks** in 25 sections (the 23 of 0.4.0 + mod-load, mod-play) ALL CHECKS PASSED; Release configure+build clean, 0 project warnings (2 upstream libopenmpt warnings ignored); Standalone smoke-launches; `--probe` verified on real MOD / XM / IT files (scratch/modules, untracked). No `gui` stage (D-1). |
